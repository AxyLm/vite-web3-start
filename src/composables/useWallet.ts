...
export function useWallet(options: useWalletOptions = { useEthers: true }) {
  ...
  async function connectWith(connector: Connector) {
    ...
    // 2. activate ethers
    try {
      setProvider(new providers.Web3Provider(wallet.provider));
    ...
    if (wallet.connector) {
      wallet.connector.onDisconnect((...args: any[]) => {
        console.log('VUE_DAPP__hasDisconnected');
        console.log(...args);
        callbacks.onDisconnectCallback && callbacks.onDisconnectCallback!(...args);
        /**
    ...
    if (wallet.connector) {
      wallet.connector.onAccountsChanged(async (accounts: string[]) => {
        callbacks.onAccountsChangedCallback && callbacks.onAccountsChangedCallback!(accounts);
        if (options.useEthers) {
          await reactivate();
        }
      });
    ...
    if (wallet.connector) {
      wallet.connector.onChainChanged(async (chainId: number) => {
        callbacks.onChainChangedCallback && callbacks.onChainChangedCallback!(chainId);
        if (options.useEthers) {
          await reactivate();
        }
      });
  ...
  async function autoConnect(connectors: Connector[]) {
    if (persistDisconnect.value && localStorage.getItem('VUE_DAPP__hasDisconnected')) {
      !dumb.value && console.warn('No auto-connect: has disconnected'); // eslint-disable-line
      return;
    }
    // try auto-connect to safe
    // const safe = connectors.find((conn) => conn.name === 'safe') as SafeConnector | undefined;
