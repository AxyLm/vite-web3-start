...
import { NETWORK_DETAILS } from '~/web3/constants/chains';
...
async function activate(externalProvider: ExternalProvider) {
  ...
  async function getData(timeout = 5000) {
    return Promise.race([
      Promise.all([_provider.getNetwork(), _signer.getAddress(), _signer.getBalance()]),
 ...
 * @returns string
 */
async function lookupDNS(chainId: number, address: string, _provider?: Web3Provider) {
  try {
    switch (chainId) {
      /*  Ethereum  */
      case 1:
